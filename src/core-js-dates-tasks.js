...
function getNextFriday(date) {
  ...
  const daysUntilFriday = 5 - dayIndex + (dayIndex < 5 ? 0 : 7);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const startDateMs = new Date(dateStart).getTime();
  const endDateMs = new Date(dateEnd).getTime();
  const msInDay = 1000 * 60 * 60 * 24;
  const difference = endDateMs - startDateMs;
  const result = Math.floor(difference / msInDay) + 1;
  return result;
}
...
function formatDate(date) {
  ...
  return `${month}/${day}/${year}, ${hours <= 12 ? hours : hours - 12}:${minutes.toString().length > 1 ? minutes : `0${minutes}`}:${seconds.toString().length > 1 ? seconds : `0${seconds}`} ${isAM ? 'AM' : 'PM'}`;
}
...
function getWeekNumberByDate(date) {
  ...
  for (let month = 0; month <= targetMonth; month += 1) {
    ...
    for (let day = 1; day <= daysInMonth; day += 1) {
      if (month === targetMonth && day > targetDay) break;

      if (new Date(targetYear, month, day).getDay() === 1) {
        weekCount += 1;
      }
    }
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const { start: startDateStr, end: endDateStr } = period;
